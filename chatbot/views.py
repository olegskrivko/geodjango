...
class ChatBotAPIView(APIView):
    ...
    def post(self, request):
        ...
        system_prompt = """
        You are an assistant for a lost pet search website. Your task is to classify the user's message into one of two categories:

        - "informational": The user is asking general questions about pets, care, adoption, or lost/found procedures.
        - "search": The user is looking to find a pet using specific filters.

        If the intent is "search", extract a JSON object with the following structure:
        ...
        - color: black, gray, white, cream, yellow, golden, brown, red, lilac, blue, green, khaki, beige, fawn, chestnut
        - gender: male, female
        - age: young, adult, senior
        - pattern: solid, striped, spotted, patched, marbled
        - recent_days: number of days since the pet was reported (e.g. 7)

        If the message includes a vague or relative time expression (like "last week", "a few days ago", or "recently"), estimate and convert it into an integer for "recent_days". For example:
        - "last week" → 7
        - "a few days ago" → 3
        - "recently added" → 2
        - "today" → 0
        ...
        Always include the "recent_days" field if there is any time reference in the message.

        If the message is not a search, return:
        { "intent": "informational" }

        Only return a clean JSON response. Do not include explanations, notes, or formatting outside the JSON.
        """
        try:
    ...
    def search_pets(self, filters):
        ...
        qs = Pet.objects.filter(is_public=True, is_banned=False, is_archived=False)
        print("Initial queryset count:", qs.count())

        # Define model field mappings from user-friendly names
        field_choice_map = {
            "status": ("status", Pet.STATUS_CHOICES),
            "species": ("species", Pet.SPECIES_CHOICES),
            "size": ("size", Pet.SIZE_CHOICES),
            "gender": ("gender", Pet.GENDER_CHOICES),
            "age": ("age", Pet.AGE_CHOICES),
            "pattern": ("pattern", Pet.PATTERN_CHOICES),
            "final_status": ("final_status", Pet.FINAL_STATUS_CHOICES),
        }

        # Loop through each supported filter
        for filter_key, (model_field, choices) in field_choice_map.items():
            if filter_key in filters:
                value_map = {name.lower(): id for id, name in choices}
                user_val = filters[filter_key].lower()
                mapped_val = value_map.get(user_val)
                if mapped_val:
                    qs = qs.filter(**{model_field: mapped_val})
                    print(f"Filtered by {filter_key}: {user_val} → {mapped_val}")
                else:
                    print(f"Unknown value for {filter_key}: {user_val}")

        # Handle color specially: match either primary or secondary
        if "color" in filters:
            color_value_map = {name.lower(): id for id, name in Pet.COLOR_CHOICES}
            color_input = filters["color"].lower()
            color_id = color_value_map.get(color_input)
            if color_id:
                qs = qs.filter(models.Q(primary_color=color_id) | models.Q(secondary_color=color_id))
                print(f"Filtered by color (primary or secondary): {color_input} → {color_id}")
            else:
                print(f"Unknown color: {color_input}")

        # Handle recent_days filter
        if "recent_days" in filters:
            try:
                days = int(filters["recent_days"])
                since = timezone.now() - timezone.timedelta(days=days)
                qs = qs.filter(created_at__gte=since)
                print(f"Filtered by recent_days: last {days} days")
            except ValueError as e:
                print(f"Invalid recent_days value: {filters['recent_days']}")
