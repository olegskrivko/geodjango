...
# from services.models import Service, Location, Review, WorkingHour, SocialMedia, ServiceCategory
...
#             self.stdout.write(self.style.ERROR("❌ No ServiceCategory objects found. Please populate ServiceCategory first."))
#             return
...
#             selected_service_categories = random.sample(service_categories, k=random.randint(1, min(3, len(service_categories))))
#             service.service_categories.add(*selected_service_categories)
...
#         )
# services/management/commands/generate_services.py
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for _ in range(20):  # Create 20 services
            ...
            country_code = random.choice(dialing_codes)
            national_number = ''.join([str(random.randint(0, 9)) for _ in range(8)])
            email = fake.company_email()
            website_url = fake.url()
            ...
            # ✅ Now add multiple categories
            selected_service_categories = random.sample(
                service_categories, k=random.randint(1, min(3, len(service_categories)))
            )
            service.service_categories.add(*selected_service_categories)
            ...
            for _ in range(num_reviews):
                ...
                if not available_users:
                    break
                review_user = random.choice(available_users)
                reviewed_users.add(review_user)
                Review.objects.create(
                    service=service,
                    user=review_user,
                    rating=Decimal(str(random.randint(1, 5))),
        ...
        self.stdout.write(
            self.style.SUCCESS("✅ Successfully created 20 services with multiple categories, locations, and reviews!")
        )
